...
class TabEBM:
    ...
    def generate(
        self,
        X, y,                            # The data must have been processed using TabEBM.add_surrogate_negative_samples()
    ...
    def _sampling_internal(
        self,
        X, y,                            # The data must have been processed using add_surrogate_negative_samples()
        ...
        if return_gradients_energy_surface:
            assert return_trajectory, "If return_gradients_energy_surface is True, then return_trajectory must be True to get the trajectory of the gradients"
        ...
        for target_class in np.unique(to_numpy(y)):
            ...
            X_ebm, y_ebm = TabEBM.add_surrogate_negative_samples(X_one_class, distance_negative_class=distance_negative_class)
            ...
            x = X_one_class[np.random.choice(len(X_one_class), size=num_samples)]   # Select random samples from the training set
            ...
            for t in range(sgld_steps):
                ...
                logits = self.tabpfn.predict_proba(x, return_logits=True)  # ====== NOTE: tabpfn.predict_proba() internally sets the seed to 0
                energy = TabEBM.compute_energy(logits)
                total_energy = energy.sum() / len(x)
                total_energy.backward()

                if debug:
                    print(f"Step {t} has energy {total_energy.item():.3f} with gradient norm {x.grad.norm().item():.4f}")
            ...
            if return_trajectory:
                res = {'sampling_paths': np.array(sgld_sampling_paths).transpose(1, 0, 2)}                    # shape = (num_samples, num_steps, num_features)
                
                if return_gradients_energy_surface:
                    res['gradients_energy_surface'] = np.array(gradients_energy_surface).transpose(1, 0, 2)   # shape = (num_samples, num_steps, num_features)
...
def plot_TabEBM_energy_contour(
        tabebm,
        X, y,                           # Point to compute the energy contours
    ...
    if show_contours:
        ...
        if type(all_logits) == torch.Tensor:
        ...
        if show_unnormalized_prob:
            title = (f'The unnormalized $p(x)$')
        else:
            title = (f'The energy of $p(x)$')

        ax.contourf(xx, yy, energy.reshape(xx.shape), cmap=cmap, alpha=alpha_contourf, levels=number_contours)

        if show_colorbar:
            cbar = plt.colorbar(ax.contourf(xx, yy, energy.reshape(xx.shape), cmap=cmap, alpha=.8),
                                ax=ax,
                                fraction=0.046,
                                pad=0.04)

            if show_unnormalized_prob:
                cbar.ax.set_ylabel(f'Unnormalized density (blue regions have high density)')
            else:
                cbar.ax.set_ylabel(f'Energy (red regions have high density)')
    ...
    if show_scatter_points:
        if color_all_scatter_points_in==None:
        ...
        if show_legend:
            ...
            legend_handles = [
                plt.Line2D([0], [0],
                           ...
                           markeredgewidth=2,
                           markeredgecolor='black') for label, color in zip(unique_labels, colors)
            ]
